...
use tuwunel_core::{Result, debug, info, utils::stream::TryExpect, warn};
...
pub(super) async fn fix_referencedevents_missing_sep(services: &Services) -> Result {
	...
	{
		let stream = referencedevents
			.raw_stream()
			.expect_ok()
			.enumerate();
		futures::pin_mut!(stream);
		while let Some((i, (key, val))) = stream.next().await {
			...
			if !has_sep {
				...
				referencedevents.put_raw(new_key, val).await?;
				referencedevents.remove(key).await?;
			}
