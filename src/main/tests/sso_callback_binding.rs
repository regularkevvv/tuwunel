...
//!   `BASE64URL(SHA256(code_verifier))` equals that challenge. A normal sign-in
//!   succeeding proves the homeserver sends a matching S256 pair; a code
//!   obtained for one sign-in and redeemed through another sign-in's callback
//!   is refused by the provider, and no login token is issued.
...
fn sso_callbacks_bind_state_cookie_and_pkce_to_one_sign_in() -> Result {
	...
	let result: Result = runtime.block_on(async {
		...
		drop(listener);
		let exercise = async {
			let outcome = timeout(Duration::from_mins(2), exercise(&client, &base, &fixture))
				.await
...
async fn exercise(client: &Client, base: &str, fixture: &Shared) -> Result {
	while client
	...
	// The homeserver asks for S256 PKCE, and a sign-in completes against a
	// provider that enforces it: the verifier it sends matches its challenge.
	let first = start(client, base).await?;
	assert_eq!(query_value(&first.authorize, "code_challenge_method")?, "S256");
	assert_eq!(
		query_value(&first.authorize, "code_challenge")?.len(),
		43,
